...
$lines = @(
    ...
    "    MemoryMaxTokens = `"`"",
    "    # MemoryNoThink  `"`" | select | all. Asks the chat template to skip the",
    "    # thinking phase on memory calls - the agent keeps thinking either way.",
    "    #   select  the faculties that CHOOSE (curator, segmenter). They pick",
    "    #           from a numbered list into a shape a schema already forces,",
    "    #           so deliberation adds little, and a bad pick costs one turn.",
    "    #   all     also the ones that WRITE (consolidator, abstractor,",
    "    #           reconsolidator). Those generalise, and a bad rule is stored,",
    "    #           recalled later, and compounds instead of expiring.",
    "    # It changes the judgement, not only the speed - do not compare runs",
    "    # made on either side of this switch.",
    "    MemoryNoThink   = `"`"",
    "    Timeout        = `"`"",
    "",
    "    # The recall funnel. The prefilter reads the WHOLE store cheaply and",
    "    # hands the curator only this many; the curator then judges. Ten out of",
    "    # thirty is a different funnel from ten out of three hundred, so the",
    "    # right width is a property of your store, not a constant.",
    "    #   CuratorEpisodes   how many memories reach the curator",
    "    #   CuratorRecent     how many of those slots are held for the newest,",
    "    #                     whatever they are about - keywords cannot answer",
    "    #                     a question about a time",
    "    #   CuratorLearnings  how many rules reach it",
    "    #   CuratorFragments  how many it may place on the desk",
    "    # Wider costs prompt on every turn, and that prompt is the latency you",
    "    # feel before the first step. `"`" = the repository default.",
    "    CuratorEpisodes  = `"`"",
    "    CuratorRecent    = `"`"",
    "    CuratorLearnings = `"`"",
    "    CuratorFragments = `"`"",
