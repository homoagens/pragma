...
$lines = @(
    ...
    "    MemoryMaxTokens = `"`"",
    "    # MemoryNoThink `"1`" asks the chat template to skip the thinking phase",
    "    # on memory calls only - the agent keeps thinking. Worth it on a model",
    "    # that ruminates: a curator picks from a numbered list and its shape is",
    "    # already forced by a JSON schema, so there is little to deliberate.",
    "    # It changes the judgement, though, not only the speed - do not compare",
    "    # runs made on either side of this switch.",
    "    MemoryNoThink   = `"`"",
    "    Timeout        = `"`"",
    "",
