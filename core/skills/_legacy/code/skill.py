...
# skills/code/skill.py — delegate a coding task to a specialized coding model
#
...
def code(task: str,
    ...
    context  : existing code or spec text to pass as context
    mode     : "generate" | "review" | "explain" | "refactor" | "fix"

    Uses CODING_* config if set, otherwise falls back to the default model.
    Returns the code as a string, or "ERROR: ..." on failure.
    """
    if not task or not task.strip():
        return "ERROR: code() requires a non-empty `task`."

    # Late imports to avoid circular loading
    import config
    import llm_client
...
def _strip_code_fences(text: str) -> str:
    """Remove ``` fences and the opening language tag if present."""
    s = (text or "").strip()
    if not s.startswith("```"):
        return s
    lines = s.splitlines()
